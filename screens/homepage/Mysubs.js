  ...
  const play = (item)=>{
    ...
    for(var i=0; i<item.info.length; i++){
      ...
      if(item.info.length==2){
        ...
        global.volume1="0."+item.info[0].audio_type.volume;
        global.type1=item.info[0].audio_type.name;
        global.track2=item.info[1].track_id;
        global.volume2="0."+item.info[1].audio_type.volume;
        ...
        global.volume3="0."+item.info[2].audio_type.volume;
        ...
        global.volume4="0."+item.info[3].audio_type.volume;
    ...
    return (
      ...
      <TouchableOpacity onPress={()=> play(item)} onLongPress={()=> addToPlaylist(item)} style={{ width: Dimensions.get('window').width/2-25,borderRadius: 20, }}>
        ...
        <TouchableOpacity style={{}}>
          <Image source={{uri: "https://cdn-icons-png.flaticon.com/512/2311/2311524.png"}} style={{width: 40, height: 40,alignSelf: 'flex-end', marginTop: -Dimensions.get('window').width/2.9, tintColor: 'white', shadowColor: 'black', shadowOpacity: 2}}/>

        </TouchableOpacity>

        <View style={{marginTop:-53}}>
          <Text numberOfLines={1} style={{ marginTop: 8, paddingLeft: 10, paddingRight: 10,color: '#0D0D0D', fontWeight: 'bold',   fontSize: 14, flex:1}}>{item.title}</Text>
        </View>
        <View style={{width: Dimensions.get('window').width/2-25,justifyContent: 'space-between', flexDirection: 'row'}}>
          <Text numberOfLines={1} style={{marginTop: 3, paddingLeft: 10, color: 'black',   fontSize: 10, width: Dimensions.get('window').width/2-68 }}>{item.category.name}</Text>
          <Text style={{marginTop: 3, paddingRight: 10, color: 'black',   fontSize: 10, marginBottom: 10 }}>1:00</Text>
