  ...
  const play=()=>{
      ...
      for(var i=0; i<global.list[0].info.length; i++){
        if( global.list[0].info.length==2){
          ...
          global.volume3="0."+global.list[0].info[2].volume;
          ...
          global.volume4="0."+global.list[0].info[3].volume;
  ...
  return(
    <ImageBackground source={require('../../assets/homebg.png')} style={{width: width, height: height}}>
      <View style={{flexDirection: 'row', justifyContent: 'space-between',}}>
        <TouchableOpacity onPress={() => navigation.navigate("Today1")} style={{width: 40, height: 40, left: 10,marginTop: 48}} >
          <Image source={require('../../assets/pageback.png')} style={{width: 26, height: 26, marginLeft: 10}} />
        </TouchableOpacity>
          {error ? (<Text style={{color: color, fontSize: 15, marginTop: 54, textAlign: 'center', marginBottom: -10}}>{error}</Text>):null}
          {isLiked1()}
      </View>
      <View style={{alignItems: 'center', justifyContent: 'center'}}>
        <Image source={{uri: global.playlist.cover}} style={{width: width/2.5, height: width/2.5, borderRadius: 20}} />
        <Text numberOfLines={1}style={{color: '#0D0D0D', fontWeight: 'bold',   fontSize: 20, marginTop: 10, textAlign: 'center' }}>{global.playlist.title}</Text>
