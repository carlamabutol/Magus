...
const TodayPlaylist =({navigation})=>{
    ...
    body: JSON.stringify({featured_id: global.playlist.featured_id, playlist_id: item.playlist_id,
      ...
      .then((result) =>{
        ...
        if(item.subliminal_count==result.data[index].subliminal_count){
          fetchData()
          setColor('red')
          return updateError("Subliminal is already part of that playlist!", setError);
        }else{
          fetchData()
          setColor('white')
  ...
  return(
    <ImageBackground source={require('../../assets/homebg.png')} style={{width: width, height: height}}>
      ...
      {error ? (<Text style={{color: color, fontSize: 15, marginTop: -10, textAlign: 'center', marginBottom: -10}}>{error}</Text>):null}
      <View style={{height: space(), alignItems: 'center', justifyContent: 'center'}}>
